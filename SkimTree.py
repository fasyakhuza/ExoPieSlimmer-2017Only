...
def runbbdm(txtfile):
    ...
    outTree.Branch('st_nVtx', st_nVtx, 'st_nVtx/L')
    outTree.Branch('mcweight', mcweight, 'mcweight/F')
    #outTree.Branch('st_mcWeight', st_mcWeight, 'st_mcWeight/L')
    ...
    outTree.Branch('st_THINjetTrackdxyError', st_THINjetTrackdxyError)
    outTree.Branch('st_THINjetTrackPt', st_THINjetTrackPt)
    outTree.Branch('st_THINjetTrackEta', st_THINjetTrackEta)
    ...
    outTree.Branch('st_eleIsPassMedium', st_eleIsPassMedium)
    ...
    outTree.Branch('st_isSoftLooseIsoMuon', st_isSoftLooseIsoMuon)

    #outTree.Branch( 'st_HPSTau_n', st_HPSTau_n, 'st_HPSTau_n/L')
    outTree.Branch('st_nTau_DRBased_EleVeto',
                   st_nTau_DRBased_EleVeto, 'st_nTau_DRBased_EleVeto/L')
    ...
    for df in read_root(filename, 'tree/treeMaker', columns=jetvariables, chunksize=125000):
        if runOn2016:
            ...
            var_zip = zip(df.runId, df.lumiSection, df.eventId, df.isData, df.mcWeight, df.mass_A, df.mass_a,
                          ...
                          df.pu_nTrueInt, df.pu_nPUVert, df.nVtx,
                          df.hlt_trigName, df.hlt_trigResult, df.hlt_filterName, df.hlt_filterResult, df.pdfscaleSysWgtID_, df.pdfscaleSysWeights,
                          df.pfmodifiedMetCorrPt, df.pfmodifiedMetCorrPhi, #df.pfmodifiedMetCorrUnc, df.pfpatmodifiedMet_smear, 
                          df.pfmodifiedMetCorrSig, df.pfpatCaloMETPt, df.pfpatCaloMETPhi, df.pfTRKMETPt_, df.pfTRKMETPhi_, df.pfMetRawPt, df.pfMetRawPhi,
                          df.nEle, df.elePx, df.elePy, df.elePz, df.eleEnergy, df.eleIsPassVeto, df.eleIsPassLoose, df.eleIsPassTight, df.eleD0, df.eleDz, df.eleIsPassMedium,
                          df.eleCharge, df.nPho, df.phoPx, df.phoPy, df.phoPz, df.phoEnergy, df.phoIsPassLoose, df.phoIsPassTight,
                          df.nMu, df.muPx, df.muPy, df.muPz, df.muEnergy, df.isLooseMuon, df.isTightMuon, df.PFIsoLoose, df.PFIsoMedium, df.PFIsoTight, df.PFIsoVeryTight, df.muCharge, df.isSoftMuon,
                          df.HPSTau_n, df.HPSTau_Px, df.HPSTau_Py, df.HPSTau_Pz, df.HPSTau_Energy, df.disc_decayModeFinding, df.disc_decayModeFindingNewDMs, df.disc_byVTightDeepTau2017v2p1VSjet, #df.disc_byLooseIsolationMVArun2017v2DBoldDMwLT2017, df.disc_byMediumIsolationMVArun2017v2DBoldDMwLT2017, df.disc_byTightIsolationMVArun2017v2DBoldDMwLT2017,
                          df.disc_againstMuonLoose3, df.disc_againstMuonTight3, df.disc_againstElectronLooseMVA6, df.disc_againstElectronMediumMVA6, df.disc_againstElectronTightMVA6,
                          df.nGenPar, df.genParId, df.genMomParId, df.genParSt, df.genParPx, df.genParPy, df.genParPz, df.genParE,
                          df.THINnJet, df.THINjetPx, df.THINjetPy, df.THINjetPz, df.THINjetEnergy, df.THINbRegNNResolution, df.THINbRegNNCorr,df.THINisPUJetIDLoose,df.THINisPUJetIDMedium,df.THINisPUJetIDTight,
                          df.THINjetPassIDTight, df.THINjetDeepCSV_b, df.THINjetHadronFlavor, df.THINjetCEmEF, df.THINjetCHadEF, df.THINjetNEmEF, df.THINjetNHadEF, df.THINjetCMulti, df.THINjetNMultiplicity, df.THINjetCorrUncUp, df.THINjetNPV, df.THINjetUncSources, df.THINjetUncTotal, df.THINjetCISVV2,
                          ...
                          df.FATjet_prob_bbvsLight, df.FATjet_prob_ccvsLight, df.FATjet_prob_TvsQCD, df.FATjet_prob_WvsQCD, df.FATjet_prob_ZHbbvsQCD,
                          df.FATjetSDmass, df.FATN2_Beta1_, df.FATN2_Beta2_, df.FATjetCHSPRmassL2L3Corr, df.FATjetCHSSDmassL2L3Corr, df.FATjetTau1, df.FATjetTau2, df.FATjetUncSources, df.FATjetUncTotal,
                          df.THINjetNTracks, df.THINjetTrackImpdz, df.THINjetTrackImpdzError, df.THINjetTrackImpdxy, df.THINjetTrackImpdxyError, df.THINjetTrackPt, df.THINjetTrackEta, df.THINjetTrackPhi, df.THINjetTrackStatus, df.THINjetTrackHighPurity)
        elif runOn2018:
            if ('mass_A' not in df.columns) and('mass_a' not in df.columns):
                df['mass_A'] = 0
                df['mass_a'] = 0
        ...
        for run, lumi, event, isData, mcWeight_, mass_A_, mass_a_,\
                ...
                type1met_, type1metphi_,\
                metCorrSig, patCaloMETPt, patCaloMETPhi, TRKMETPt_, TRKMETPhi_, MetRawPt, MetRawPhi,\
                nele_, elepx_, elepy_, elepz_, elee_, elevetoid_, elelooseid_, eletightid_, eleD0_, eleDz_, elemediumid_,\
                eleCharge_, npho_, phopx_, phopy_, phopz_, phoe_, pholooseid_, photightID_,\
                nmu_, mupx_, mupy_, mupz_, mue_, mulooseid_, mutightid_, muisoloose, muisomedium, muisotight, muisovtight, muCharge_, musoftid_,\
                nTau_, tau_px_, tau_py_, tau_pz_, tau_e_, tau_dm_, tau_dmNewDMs_, tau_VTight_,\
                ...
                ak4PassID_, ak4deepcsv_, ak4flavor_, ak4CEmEF_, ak4CHadEF_, ak4NEmEF_, ak4NHadEF_, ak4CMulti_, ak4NMultiplicity_, ak4JEC_, ak4NPV_, ak4jetUncSources, ak4jetUncTotal, ak4jetCISVV2,\
            ...
            *      *   *****
            ...
            mu_pt20_eta2p4_softID_looseISO = boolutil.logical_and4((mupt > 20.0), (numpy.abs(mueta) < 2.4), (musoftid_), (muisoloose))

            pass_mu_index = boolutil.WhereIsTrue(
                mu_pt10_eta2p4_looseID_looseISO)
            pass_muSoft_index = boolutil.WhereIsTrue(mu_pt20_eta2p4_softID_looseISO)
            ...
            ***       *****      *
            ...
            #ak4_pt30_eta2p5 = [((ak4pt[ijet] > 30.0) and (numpy.abs(ak4eta[ijet]) < 2.5)) for ijet in range(nak4jet_)]
            ak4_pt30_eta2p5 = boolutil.logical_and2((ak4pt > 30.0), (numpy.abs(ak4eta) < 2.5))
            ...
            if len(ak4_pt30_eta2p5) > 0:
                ...
                jetCleanAgainstEle = anautil.jetcleaning(ak4_pt30_eta2p5, ele_pt20_eta2p4_mediumID, ak4eta, eleeta, ak4phi, elephi, DRCut)
                jetCleanAgainstMu = anautil.jetcleaning(ak4_pt30_eta2p5, mu_pt20_eta2p4_softID_looseISO, ak4eta, mueta, ak4phi, muphi, DRCut)
                
                #jetCleaned = boolutil.logical_AND_List3(
                #    ak4_pt30_eta2p5, jetCleanAgainstEle, jetCleanAgainstMu)
                jetCleaned = boolutil.logical_and2(ak4_pt30_eta2p5, jetCleanAgainstEle)
            ...
            ********    *        *       *
            ...
            tau_pt20_eta2p5_iDLdm_vtight = boolutil.logical_AND_List4((taupt > 20.0), (numpy.abs(taueta) < 2.5), (tau_dmNewDMs_), (tau_VTight_))

            if debug_:
                #print "tau_eta2p3_iDLdm_pt18 = ", tau_eta2p3_iDLdm_pt18
                print "tau_eta2p3_iDLdm_pt18 = ", tau_pt20_eta2p5_iDLdm_vtight
            ...
            if len(tau_pt20_eta2p5_iDLdm_vtight) > 0:
                DRCut = 0.4
                ...
                tauCleanAgainstEle = anautil.jetcleaning(tau_pt20_eta2p5_iDLdm_vtight, ele_pt20_eta2p4_mediumID, taueta, eleeta, tauphi, elephi, DRCut)
                
                #tauCleaned = boolutil.logical_AND_List3(
                #    tau_eta2p3_iDLdm_pt18, tauCleanAgainstEle, tauCleanAgainstMu)
                tauCleaned = boolutil.logical_and2(tau_pt20_eta2p5_iDLdm_vtight, tauCleanAgainstEle)
            ...
            st_THINjetTrackdxyError.clear()
            st_THINjetTrackPt.clear()
            st_THINjetTrackEta.clear()
            ...
            #st_METXYCorr_Met.clear()
            #st_METXYCorr_MetPhi.clear()
            #st_genParPt.clear()
            ...
            for ithinjet in pass_jet_index_cleaned:
                ...
                st_THINjetCISVV2.push_back(ak4jetCISVV2[ithinjet])
                ...
                *********  ******     *    *
                ...
                temp_floatVecTrackdxy.clear()
                temp_floatVecTrackdxyError.clear()
                temp_floatVecTrackPt.clear()
                ...
                st_THINjetTrackdxyError.push_back(temp_floatVecTrackdxyError)
            ...
            for imu in pass_muSoft_index:
                ...
                st_isSoftLooseIsoMuon.push_back(bool(mu_pt20_eta2p4_softID_looseISO[imu]))
            if debug_:
                print 'nMu: ', len(pass_muSoft_index)

            st_nTau_DRBased_EleVeto[0] = len(pass_tau_index_cleaned_DRBased)
